...
There are 6 attributes given in the dataset which are:

1.NoteID : An identifier that uniquely identifies each note of a patient. It is a number represented as an integer

2.Description: A detailed description of the patient's note while visiting the hospital along with their problem

3.Medical Speciality:  A character variable indicating the medical specialty associated with the note or transcription. Eg: Cardiology

4.Sample Name : Depicts the sample name that is captured in the hospital and is associated with a note and provide extra information about their condition and the case.

5.Transcription : Transcribed version of the note containing subjective and objective data,patients condition,symptoms and other important information. Can be a string

6.Keywords : Identifies the various concepts/keywords associated with the transcription.It provides a list of terms that describe the medical conditions, procedures, or other relevant topics discussed in the text of the note or transcription.
...
The unique unigrams pertaining to each speciality are as follows:
...
The unique bi-grams pertaining to each speciality are as follows:
1.Orthipedic : 55732
2.Radiology : 28297
3.Surgery : 130404
...
```{r}
sentences_data <- tokenized.data.unigram %>%
  unnest_tokens(sentence,sample_name,token = "sentences")

unique_sentences <- sentences_data %>%
...
A generalized lemma might not work as efficiently as a specialized one for medical related data. It could be due to the following reasons:

1.Variations in medical terms : Medical terms are most often expressed using different synonyms,acronyms and abbreviations and a general purpose lemma might not be able to lemmatize those terminologies.

2.Uncommon Terminologies : Medical data might contain terms which are extremely specific to each speciality and such concepts might not be present in a general purpose lemma leading to such concepts being excluded.
...
The Y -axis represents the lemma percentage representing the Orthopedic speciality.

While comparing Surgery and Orthopedic, we can see that the lemma frequencies are similar for both since all the points are clustered around the diagonal region.
However, as seen by the points straying off the line, there are minor variances. This indicates that some lemmas may be more prevalent in one speciality than the other.

While comparing the Radiology and Surgery graph, although we cannot explicitly draw a conclusion from the code since the plotted graph doesn't draw any comparison between Radiology and Surgery, we can still say that lemma frequencies are different.
But if we consider radiology to be a separate specialty, the rates of lemmas between radiology and surgery ought to be different. This is because radiology largely deals with medical imaging and diagnostics, whereas surgery entails performing surgical procedures. Each specialty's lemmas would naturally vary depending on these distinct domains of expertise.
...
Below are the lemmas that stand out in each of the mentioned Speciality:

A)Orthopedic 

 1. Range Motion: This lemma has a count of 139 which is high along with TF_IDF values. It is one of the most frequently occuring lemmas within the orthopaedic speciality.

 2. Carpal ligament and "Transverse Carpal": These lemmas have relatively high counts and TF-IDF scores with a count of 85 and 81
 
 3. Extremity prepped and "Proximal phalanx": These lemmas have moderate counts and TF-IDF scores. 

B)Radiology

 1. Myocardial perfusion: These lemmas have highest counts and TF-IDF scores. They are likely important concepts within cardiology.

 2. Motor units: These lemmas have relatively lower counts and TF-IDF scores. They could be related to neurological examinations or diagnostic procedures.

 3. Calcific Plaque: This lemma refers to the calcified deposits found in arteries which are commonly used in Radiology.

C)Surgery

 1. Anterior chamber: These lemmas have highest counts among Surgery and TF-IDF scores.

 2. Lithotomy Position: These lemmas have moderate counts and TF-IDF scores.

 3. External Oblique: This lemma have moderate count.
...
```{r}
unusual_lemma <- all.counts.tfidf %>%
  group_by(medical_specialty) %>%
  slice_min(order_by = tf_idf, n = 1)
unusual_lemma
```
...
The probabilistic distribution of topics within each specialty in the LDA model results in the division of specialties into sub-topics. Within each specialty, there is a set of probabilities that represent the likelihood of each topic's occurrence.

The distribution of gamma values, which in the LDA model represent the likelihood of each medical specialty belonging to particular themes, governs the division of specialties into subtopics. This specialty-topic distribution is found in the 'specialty_gamma_6' dataframe in the given code. These sub-topics within each specialisation are easier to discern using the box plot visualisation. 

The distribution of gamma values for each speciality over the six themes is shown by a box plot. We can see how the probability are divided among topics within each speciality by looking at the box plots. A speciality may span a variety of subtopics if it displays a large range of gamma values across several topics. On the other hand, concentrated gamma values in a small number of focused areas suggest a more specialised attention. 
